...
async function initAppData() {
  ...
  if (document.getElementById('cfg-tavily')) document.getElementById('cfg-tavily').value = CFG.tavilyKey || ''; 
...
async function handleFiles(event) {
  ...
  for(let file of files) {
    if (file.size > MAX_SIZE) {
      toast(`File ${file.name} is too large (max 2MB)`, 'err');
      continue;
    }
...
function saveConfig() {
  ...
  if (document.getElementById('cfg-tavily')) CFG.tavilyKey = document.getElementById('cfg-tavily').value.trim(); 
...
async function sendMessage() {
  ...
  // --- Tavily Search Intercept (With Caching & Toggle Opacity) ---
  ...
  if (webSearchEnabled && text) {
    ...
    if (searchCache[cacheKey]) {
      searchContext = searchCache[cacheKey];
    } else {
      const toggleUi = document.getElementById('web-search-toggle');
      if (toggleUi) toggleUi.style.opacity = '0.5'; 
      ...
      try {
        ...
        if (data.results && data.results.length > 0) {
          ...
          searchCache[cacheKey] = searchContext;
        } else {
