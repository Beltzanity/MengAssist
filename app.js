...
async function handleLogout() {
  try { await supabaseClient.auth.signOut(); } catch (error) { console.warn("Supabase signout error:", error); }
  const storedKey = localStorage.getItem('mengassist_key');
  localStorage.clear();
  if (storedKey) localStorage.setItem('mengassist_key', storedKey);
...
async function initAppData() {
  ...
  } catch (err) { console.log("No remote config found."); }
  ...
  } catch (err) { console.log("No remote chats found."); }

  if (chatToLoad) loadChat(chatToLoad, true); else startNewChat(true);
...
async function syncSettingsToDB() {
  ...
  try { await supabaseClient.from('settings').upsert({ user_id: currentUser.id, config: safeConfig }); } catch(err) { console.error(err); }
}
...
async function saveState() {
  ...
  if (chatIndex > -1) { 
    ...
    if (currentUser) {
      ...
      } catch (e) { toast("Failed to sync chat", "err"); }
    }
  }
  renderSidebar();
}
...
async function handleFiles(event) {
  ...
  for(let file of files) {
    if (file.size > 5  1024  1024) { toast(File ${file.name} is too large (max 5MB), 'err'); continue; }
...
async function applyPreset() {
  ...
  if (currentKey) { await connectAPI(); }
}
...
function renderModelInput(placeholderText = '') {
  ...
  inp.type = 'text'; inp.id = 'cfg-model'; inp.value = CFG.model || '';
...
function initPresets() {
  if (!CFG.systemPresets || CFG.systemPresets.length !== 5) {
    CFG.systemPresets = [CFG.system || 'You are a helpful assistant.', '', '', '', ''];
...
function saveSystem() {
  ...
  CFG.systemPresets[idx] = text; CFG.activePresetIdx = idx; CFG.system = text;             
  syncSettingsToDB(); closeModal(); toast('System Prompt Saved ✓', 'ok'); 
}
...
function openModal(id) {
  ...
  if (id === 'modal-sys') {
    ...
    document.getElementById('temp-temp').value = CFG.temp; document.getElementById('val-temp').textContent = CFG.temp.toFixed(1); 
    document.getElementById('temp-topp').value = CFG.topp; document.getElementById('val-topp').textContent = CFG.topp.toFixed(2); 
  }
}
...
function saveConfig() {
  ...
  syncSettingsToDB(); toggleConfig(); toast('Config saved ✓', 'ok');
}

// ─── Data Management ──────────────────────────────────────
function startNewChat(keepSidebarOpen = false) { currentChatId = null; turns = []; renderAll(); renderSidebar(); if (!keepSidebarOpen) toggleSidebar(false); }

function loadChat(id, keepSidebarOpen = false) {
  const chat = chats.find(c => c.id === id);
  if (chat) { currentChatId = id; turns = chat.turns.map(t => ({...t, _editing: false, _typing: false, _loading: false})); renderAll(); renderSidebar(); if (!keepSidebarOpen) toggleSidebar(false); }
}

function delChat(e, id) { 
  e.stopPropagation(); 
  openConfirm('Are you sure you want to permanently delete this chat history?', async () => {
    chats = chats.filter(c => c.id !== id); saveLocalChats();
    try { if (currentUser) await supabaseClient.from('chats').delete().eq('id', id); } catch(err){}
    if (currentChatId === id) { if (chats.length > 0) loadChat(chats[0].id, true); else startNewChat(true); } else renderSidebar(); 
  });
}
...
function clearCurrentChat() { 
  openConfirm('Are you sure you want to clear all messages in this chat?', async () => {
    ...
    const chatIndex = chats.findIndex(c => c.id === currentChatId);
    if (chatIndex > -1) {
      chats[chatIndex].turns = []; saveLocalChats();
      try { if (currentUser) await supabaseClient.from('chats').update({ turns: [] }).eq('id', currentChatId); } catch(err) {}
    }
    document.getElementById('config').classList.remove('open'); document.getElementById('cfg-toggle').classList.remove('active'); renderAll();
  });
}
...
function buildMessages(upToTurnIdx = null) {
  ...
  for (let i = 0; i < end; i++) {
    ...
    if (t.role === 'user') {
      ...
      } else { list.push({ role: 'user', content: promptText.trim() }); }
    } else { list.push({ role: 'assistant', content: t.versions[t.idx] }); }
  }
  return list;
}
...
async function sendMessage() {
  ...
  turns.push({ role: 'user', content: text, images: images.length > 0 ? images : null, files: textFiles.length > 0 ? textFiles : null }); 
  
  inp.value = ''; autoResize(inp); pendingAttachments = []; renderAttachments(); renderAll(); saveState();
...
async function callAPI(isRegen, regenIdx) {
  ...
  if (isRegen) turns[regenIdx]._loading = true; else turns.push({ role: 'ai', _typing: true, versions: [], thinkVersions: [], idx: 0 });
  renderAll();

  try {
    const res = await fetch(CFG.url, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json', 'Authorization': Bearer ${CFG.key}, 'HTTP-Referer': window.location.origin, 'X-Title': 'MengAssist' },
    ...
    if (!thinking && content.includes('<think>')) {
      ...
      if (match) { thinking = match[1].trim(); content = content.replace(/<think>[\s\S]*?<\/think>/, '').trim(); }
    }
    const targetIdx = isRegen ? regenIdx : turns.length - 1;
    const turn = turns[targetIdx];
    delete turn._typing; delete turn._loading;
    if (isRegen) { turn.versions.push(content); turn.thinkVersions.push(thinking); turn.idx = turn.versions.length - 1; } 
    else { turn.versions = [content]; turn.thinkVersions = [thinking]; turn.idx = 0; }
  } catch (err) {
    toast(err.message, 'err');
    if (!isRegen) turns.pop(); else delete turns[regenIdx]._loading;
  } finally {
...
async function regenMessage(idx) { const lastAiIdx = turns.findLastIndex(x => x.role === 'ai'); if (idx !== lastAiIdx) return toast('Can only regen last message', 'err'); if (loading) return; await callAPI(true, idx); }

// ─── UI Rendering ─────────────────────────────────────────
function renderAll(skipScroll = false) {
  const box = document.getElementById('chat-box'); const prevScroll = box.scrollTop; box.innerHTML = '';
  ...
  if (!skipScroll) scrollBottom(); else box.scrollTop = prevScroll; 
}
...
function makeUserMsg(t, i) {
  ...
  if (t._editing) {
    ...
    const actions = el('div', 'edit-actions'), cancelBtn = el('button'); cancelBtn.textContent = 'Cancel'; cancelBtn.onclick = () => cancelEdit(i);
    ...
    if (t.images && t.images.length > 0) { const imgWrap = el('div', 'msg-images'); t.images.forEach(src => { const img = el('img'); img.src = src; imgWrap.appendChild(img); }); body.appendChild(imgWrap); }
    if (t.files && t.files.length > 0) { const fileWrap = el('div', 'msg-images'); t.files.forEach(f => { const chip = el('div', 'attach-chip'); chip.style.color = 'var(--text)'; chip.innerHTML = 📄 ${f.name}; fileWrap.appendChild(chip); }); body.appendChild(fileWrap); }
    if (t.content) { const bubble = el('div', 'bubble'); bubble.innerHTML = fmt(t.content); body.appendChild(bubble); }
    const controls = el('div', 'msg-controls'), editBtn = el('button', 'ctrl-btn'); editBtn.textContent = '✎ edit'; editBtn.onclick = () => startEdit(i);
    controls.append(editBtn); if (t.content) controls.append(mkCopy(t.content)); body.appendChild(controls);
  }
...
function makeAiMsg(t, i) {
  ...
  if (CFG.thinking && thinking) { const wrap = el('div', 'thinking-wrap'); wrap.innerHTML = <details class="think"><summary>Reasoning</summary><div class="think-body">${esc(thinking)}</div></details>; bubble.appendChild(wrap); }
  const txt = el('span', ''); txt.innerHTML = fmt(content); bubble.appendChild(txt);
  const controls = el('div', 'msg-controls');
  if (total > 1) {
    const pager = el('div', 'ver-pager'), btnPrev = el('button', 'ctrl-btn'); btnPrev.textContent = '❮'; if (t.idx === 0) btnPrev.disabled = true; btnPrev.onclick = () => prevVersion(i);
  ...
  if (i === turns.findLastIndex(x => x.role === 'ai')) { const regenBtn = el('button', 'ctrl-btn'); regenBtn.textContent = '↻ regen'; regenBtn.onclick = () => regenMessage(i); controls.appendChild(regenBtn); }
...
async function executeCopy(text, btnNode) {
  let ok = false; if (navigator.clipboard && window.isSecureContext) { try { await navigator.clipboard.writeText(text); ok = true; } catch(e){} }
...
function buildCodeBlock(lang, code) { return <div class="code-block"><div class="code-header"><span class="code-lang">${lang || 'code'}</span><div class="code-actions"><button class="code-btn code-copy" onclick="copyCode(this)">⎘ copy</button><button class="code-btn code-collapse" onclick="toggleCode(this)">▼ collapse</button></div></div><pre><code>${code.trim()}</code></pre></div>; }
function fmt(t) {
  t = esc(t); let hasCode = false; const blockRegex = new RegExp('{3}(\\w)\\n?([\\s\\S]?){3}', 'g');
  ...
  t = t.replace(/^### (.$)/gim, '<h3>$1</h3>'); t = t.replace(/^## (.$)/gim, '<h2>$1</h2>'); t = t.replace(/^# (.*$)/gim, '<h1>$1</h1>');
  t = t.replace(/\\(.+?)\\/g, '<strong>$1</strong>'); t = t.replace(/\(.+?)\/g, '<em>$1</em>'); t = t.replace(/^\s[-] (.*)/gim, '<li>$1</li>');
  const inlineRegex = new RegExp('([^\\n]+)`', 'g'); t = t.replace(inlineRegex, '<code>$1</code>'); return t;
}
