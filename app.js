...
async function initAppData() {
  ...
  if (document.getElementById('cfg-tavily')) document.getElementById('cfg-tavily').value = CFG.tavilyKey || ''; // NEW
  ...
  } catch (err) { console.log("No remote config found or error fetching:", err.message); }
  ...
  } catch (err) { console.log("No remote chats found or error fetching:", err.message); }
...
async function syncSettingsToDB() {
  ...
  try { await supabaseClient.from('settings').upsert({ user_id: currentUser.id, config: safeConfig }); } catch(err) { console.error("Failed to sync settings:", err); }
}
...
async function handleFiles(event) {
  ...
  for(let file of files) {
    if (file.size > MAX_SIZE) {
      toast(`File ${file.name} is too large (max 2MB to prevent token overflow)`, 'err');
      continue;
    }
...
function saveSystem() {
  ...
  CFG.systemPresets[idx] = text; 
  CFG.activePresetIdx = idx;     
  CFG.system = text;             
  
  syncSettingsToDB(); closeModal(); toast('System Prompt Saved & Applied ✓', 'ok'); 
}
...
function saveConfig() {
  ...
  if (document.getElementById('cfg-tavily')) CFG.tavilyKey = document.getElementById('cfg-tavily').value.trim(); // NEW
...
function clearCurrentChat() { 
  openConfirm('Are you sure you want to clear all messages in this chat?', async () => {
    ...
    if (chatIndex > -1) {
      ...
      } catch(err) { toast("Failed to clear chat in cloud", "err"); }
    }
...
// ─── Button UI Toggle (Send <-> Stop) ─────────────────────
function toggleSendButton(isGenerating) {
  const btn = document.getElementById('send-btn');
  if (isGenerating) {
    btn.innerHTML = `<svg viewBox="0 0 24 24" fill="currentColor" stroke="none" width="12" height="12"><rect x="6" y="6" width="12" height="12" rx="2"></rect></svg>`;
    btn.classList.add('stop-mode');
    btn.onclick = stopGeneration;
    btn.disabled = false; 
  } else {
    btn.innerHTML = `<svg viewBox="0 0 24 24" fill="none" stroke="#0d0d0f" stroke-width="2.5" stroke-linecap="round" stroke-linejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>`;
    btn.classList.remove('stop-mode');
    btn.onclick = sendMessage;
    btn.disabled = filesLoadingCount > 0 || loading;
  }
}
...
async function sendMessage() {
  ...
  if (webSearchEnabled && text) {
    ...
    try {
      ...
      if (data.results && data.results.length > 0) {
        searchContext = data.results.map(r => `Title: ${r.title}\nURL: ${r.url}\nContent: ${r.content}`).join('\n\n');
      } else {
        toast('No search results found', 'err');
      }
    } catch(err) {
  ...
  turns.push({ 
    role: 'user', 
    content: text, 
    searchContext: searchContext, // Store the context
...
async function callAPI(isRegen, regenIdx) {
  ...
  const msgUpTo = isRegen ? regenIdx : null; 
  const messages = buildMessages(msgUpTo);
  
  let targetIdx;
  if (isRegen) { 
    delete turns[regenIdx]._loading; 
    turns[regenIdx].versions.push(''); 
    turns[regenIdx].thinkVersions.push(''); 
    turns[regenIdx].idx = turns[regenIdx].versions.length - 1; 
    targetIdx = regenIdx;
  } else { 
    turns = turns.filter(t => !t._typing); 
    turns.push({ role: 'ai', versions: [''], thinkVersions: [''], idx: 0 }); 
    targetIdx = turns.length - 1;
  }
  
  renderAll(); 

  const msgNodes = document.querySelectorAll('#chat-box .msg');
  const targetNode = msgNodes[targetIdx];
  const bubbleNode = targetNode.querySelector('.bubble');
...
function streamUpdateDOM(bubbleNode, content, thinking) {
  ...
  if (backtickCount % 2 !== 0) safeContent += '\n```'; 
  
  txt.innerHTML = fmt(safeContent); 
  bubbleNode.appendChild(txt);
}
...
async function submitEdit(idx, newText) { 
  ...
  turns.push({ role: 'user', content: newText.trim(), images: oldTurn.images, files: oldTurn.files, searchContext: oldTurn.searchContext }); 
...
function makeUserMsg(t, i) {
  ...
  if (t._editing) {
    ...
    // NEW: Render a chip if a web search was performed
    if (t.searchContext) {
      const searchWrap = el('div', 'msg-images'); 
      const chip = el('div', 'attach-chip'); 
      chip.style.color = 'var(--text)';
      chip.innerHTML = `🌐 Web Search Applied`; 
      searchWrap.appendChild(chip);
...
function makeAiMsg(t, i) {
  ...
  if (CFG.thinking && thinking) {
    const wrap = el('div', 'thinking-wrap');
    wrap.innerHTML = `<details class="think"><summary>Reasoning</summary><div class="think-body">${fmt(thinking)}</div></details>`;
...
function scrollBottom(smart = false) { 
  const b = document.getElementById('chat-box'); 
  if (smart) {
    const isAtBottom = b.scrollHeight - b.scrollTop - b.clientHeight < 150;
